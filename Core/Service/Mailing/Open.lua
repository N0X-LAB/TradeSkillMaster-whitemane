...
function private.OpenMailThread(autoRefresh, keepMoney, filterText, filterType)
	while true do
		...
		if autoRefresh and not filterType and filterText == "" then
			private.DeleteEmptyMails()
		end
	...
	for i = 1, #mails do
		...
		if matchesFilter and hasBagSpace then
			...
			if cod == 0 and (not keepMoney or (keepMoney and money <= 0)) then
				if money <= 0 and (not numItems or numItems == 0) then
...
function private.DeleteEmptyMail(index)
	...
	if money == 0 and (not itemCount or itemCount == 0) and InboxItemCanDelete(index) then
		DeleteInboxItem(index)
		return true
	end
end

function private.DeleteEmptyMails()
	local numItems = Inbox.GetNumItems()
	for index = numItems, 1, -1 do
		Threading.WaitForFunction(private.CanOpenMail)
		if private.DeleteEmptyMail(index) then
			Threading.WaitForFunction(private.CanOpenMail)
		end
	end
end

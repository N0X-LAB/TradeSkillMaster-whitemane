...
function AuctioningOperation.Load(localizedName, defaultZeroUndercut, includeBlacklist, maxStackSizeFunc)
	...
	local operationType = Operation.NewType(OPERATION_TYPE, localizedName, 20)
		...
		:AddCustomStringSetting("minPrice", "check(first(crafting,dbmarket),max(0.25*avg(crafting,dbmarket),1.5*vendorsell))")
		:AddCustomStringSetting("maxPrice", "check(first(crafting,dbmarket),max(5*avg(crafting,dbmarket),30*vendorsell))")
		:AddCustomStringSetting("normalPrice", "check(first(crafting,dbmarket),max(2*avg(crafting,dbmarket),12*vendorsell))")

...
local STATE_SCHEMA = Reactive.CreateStateSchema("AUCTION_BUY_SCAN_STATE")
	...
	:AddBooleanField("forceBuyConfirmation", false)
...
---@param buyConfirmedFunc fun(itemString: string, itemBuyout: number) Function called when an item is bought
---@return AuctionBuyScan
function AuctionBuyScan.__static.NewBrose(scanTypeName, isPlayerFunc, alertThresholdFunc, postUndercutFunc, postBidUndercutFunc, postQuantityFunc, buyQuantityFunc, buyConfirmedFunc)
	return AuctionBuyScan(SCAN_TYPE.BROWSE, scanTypeName, isPlayerFunc, alertThresholdFunc, postUndercutFunc, postBidUndercutFunc, postQuantityFunc, buyQuantityFunc, buyConfirmedFunc)
end
...
function AuctionBuyScan.__private:__init(scanType, scanTypeName, isPlayerFunc, alertThresholdFunc, postUndercutFunc, postBidUndercutFunc, postQuantityFunc, buyQuantityFunc, buyConfirmedFunc)
	...
	self._buyConfirmedFunc = buyConfirmedFunc
	...
	if not LibTSMUI.IsModernAuctionHouse() then
...
function AuctionBuyScan.__private:_ActionHandler(manager, state, action, ...)
	if action == "ACTION_START_SEARCH" then
		...
		if state.scanType == SCAN_TYPE.BROWSE then
			resolveSellers = not LibTSMUI.IsModernAuctionHouse()
	...
	elseif action == "ACTION_RESET_STATE" then
		...
		state.forceBuyConfirmation = false
	...
	elseif action == "ACTION_SET_SELECTED_AUCTION" then
		...
		if selection and selection:IsSubRow() then
			...
			state.selectionCanCancel = LibTSMUI.IsModernAuctionHouse() and self._isPlayerFunc(ownerStr, false)
		...
		if result then
			...
			if state.autoBuyOnFind then
				...
				if state.selectionCanBuy and state.numFound > 0 then
					state.forceBuyConfirmation = true
	...
	elseif action == "ACTION_BUY_AUCTION" then
		...
		local forceBuyConfirmation = state.forceBuyConfirmation
		state.forceBuyConfirmation = false
		if not self:_ShowConfirmation(true, forceBuyConfirmation) then
			if forceBuyConfirmation then
				return
			end
	...
	elseif action == "ACTION_BUY_AUCTION_CONFIRMED" then
		...
		state.lastBuyQuantity = 0
		state.lastBuyIndex = nil
		local index = not LibTSMUI.IsModernAuctionHouse() and tremove(state.findResult, #state.findResult) or nil
		if not LibTSMUI.IsModernAuctionHouse() and not index then
		...
		if not LibTSMUI.IsModernAuctionHouse() and buyout ~= select(5, AuctionHouse.GetBrowseResult(index)) then
		...
		state.numBought = state.numBought + (LibTSMUI.IsModernAuctionHouse() and quantity or 1)
		manager:ManageFuture("pendingFuture", future, "ACTION_BUYOUT_FUTURE_DONE")
	elseif action == "ACTION_BUYOUT_FUTURE_DONE" then
		local result = ...
		if result then
			local itemString = state.selectedAuction:GetItemString()
			Mail.HandleAuctionPurchase(ItemString.ToLevel(itemString), state.lastBuyQuantity)
			if self._buyConfirmedFunc then
				local _, itemBuyout = state.selectedAuction:GetBuyouts()
				self._buyConfirmedFunc(itemString, itemBuyout)
			end
			state.numConfirmed = min(state.numConfirmed + (LibTSMUI.IsModernAuctionHouse() and state.lastBuyQuantity or 1), state.numFound)
			...
			if state.lastBuyQuantity > 0 then
				state.numBought = state.numBought - (LibTSMUI.IsModernAuctionHouse() and state.lastBuyQuantity or 1)
				if not LibTSMUI.IsModernAuctionHouse() then
	...
	elseif action == "ACTION_REMOVE_BOUGHT_AUCTIONS" then
		...
		if selection and not selection:IsSubRow() then
			...
			local maxQuantity = state.searchContext:GetMaxCanBuy(itemString)
			if maxQuantity then
				if not LibTSMUI.IsModernAuctionHouse() and selection then
	...
	elseif action == "ACTION_BID_AUCTION_CONFIRMED" then
		...
		state.lastBuyIndex = nil
		local index = not LibTSMUI.IsModernAuctionHouse() and tremove(state.findResult, #state.findResult) or nil
		assert(LibTSMUI.IsModernAuctionHouse() or index)
		-- Bid on the auction
		...
		state.lastBuyQuantity = quantity
		state.lastBuyIndex = index
		state.numBid = state.numBid + (LibTSMUI.IsModernAuctionHouse() and quantity or 1)
	...
	elseif action == "ACTION_CANCEL_AUCTION" then
		assert(LibTSMUI.IsModernAuctionHouse() and state.selectedAuction and state.selectedAuction:IsSubRow())
...
function AuctionBuyScan.__private:_ShowConfirmation(isBuy, forceConfirmation)
...
function private.GetConfirmationDialog(state, isBuy, alertThreshold)
	...
	local isCommodity = LibTSMUI.IsModernAuctionHouse() and state.selectedAuction:IsCommodity()
